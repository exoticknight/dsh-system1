...
export async function execute(signals, timeoutMs, operation) {
    const controller = new AbortController();
    const deadline = performance.now() + timeoutMs;
    const listeners = [];
    let timer;
    let abortListener;
    const cancel = () => controller.abort(new System1ProviderError({
        code: 'cancelled',
    ...
    try {
        ...
        const timeout = () => controller.abort(new System1ProviderError({
            code: 'timeout',
